 ...
 *
 * Generates accountant-ready exports from the ledger.
 * Formats:
 *   - Ledger CSV: ref_number, date, entity, type, amount, status
 *   - Journal CSV: ref_number, date, entity, account_code, account_name, debit, credit
 *   - JSON: full extraction data for custom processing
 */
...
export class ExportService {
  ...
  async exportLedgerCSV(params: { dateFrom?: string; dateTo?: string; status?: string }): Promise<string> {
    ...
    const header = 'ref_number,date,entity,entry_type,amount,debit_amount,credit_amount,balance_type,status,created_at\n';
  ...
  async exportJournalCSV(params: { dateFrom?: string; dateTo?: string }): Promise<string> {
    let query = `
      SELECT
        je.ref_number, je.entry_date, le.entity, je.doc_type, je.status,
    ...
    const header = 'ref_number,date,entity,doc_type,status,account_code,account_name,debit,credit,memo\n';
    const lines = rows.map(r =>
      [
        r.ref_number,
        r.entry_date,
  ...
  async exportJSON(params: { dateFrom?: string; dateTo?: string }): Promise<object> {
    const ledger = await this.db.prepare(
      'SELECT * FROM ledger_entries ORDER BY date ASC'
    ).all();
    const extractions = await this.db.prepare(
      'SELECT * FROM extractions ORDER BY extracted_at ASC'
    ).all();
    return {
