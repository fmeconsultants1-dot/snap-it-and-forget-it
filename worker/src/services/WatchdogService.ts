/**
 * WatchdogService.ts - FME Mission 001
 * Health monitoring: stuck runs, failed docs, unbalanced journals, missing ledger entries
 */
import type { Env } from './ScanService';
...
export class WatchdogService {
  ...
  async check(): Promise<HealthReport> {
    ...
    try { await this.db.prepare('SELECT 1').first(); dbOk = true; }
    catch { issues.push('D1 database unreachable'); }

    try { await this.r2.list({ limit: 1 }); r2Ok = true; }
    catch { issues.push('R2 bucket unreachable'); }
    ...
    if (dbOk) {
      const stuck = await this.db.prepare(
        "SELECT COUNT(*) as cnt FROM scan_runs WHERE status='PROCESSING' AND created_at < datetime('now','-10 minutes')"
      ).first() as any;
      ...
      const ml = await this.db.prepare(
        "SELECT COUNT(*) as cnt FROM documents d WHERE d.status='DONE' AND NOT EXISTS (SELECT 1 FROM ledger_entries le WHERE le.document_id=d.id)"
      ).first() as any;
    ...
    return {
      status, db: dbOk, r2: r2Ok,
      gemini_configured: geminiConfigured,
      stuck_runs: stuckRuns, failed_docs: failedDocs,
      unbalanced_entries: unbalanced, missing_ledger_entries: missingLedger,
