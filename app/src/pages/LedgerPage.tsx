 ...
 * Register + Accounting Journal, all tabs, Approve, Source, Refund, Split.
 * Mobile fix: overflow-x hidden, min-width:0 on all grid children, tab-bar scrolls internally.
 */
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/refund`, {
        ...
        body: JSON.stringify({
          originalLedgerEntryId: entry.id, refundType: type,
          refundAmount: parseFloat(amount), refundDate: date,
          memo: memo || undefined, idempotencyKey: idemKey,
        }),
      });
      ...
      const res = await fetch(`${API_URL}/api/ledger/${entry.id}/split`, {
        ...
        body: JSON.stringify({
          splits: lines.map(l => ({
            ...
            expense_account_name: accMap[l.account_code] ?? l.account_name,
      ...
      <div style={{ ...modal, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          {([['GST', gst, setGst], ['HST', hst, setHst], ['PST', pst, setPst]] as const).map(([l, v, fn]) => (
            <div key={l} style={{ flex: 1 }}>
              ...
              <input type="number" step="0.01" value={v} onChange={e => (fn as any)(e.target.value)} style={inp} />
            </div>
          ))}
        </div>
        ...
        {lines.map((line, i) => (
          <div key={i} style={{ background: 'var(--bg-card-2)', borderRadius: 8, padding: 10, marginBottom: 8 }}>
            ...
            <input placeholder="Description" value={line.description} style={{ ...inp, marginBottom: 4 }}
              onChange={e => setLines(p => p.map((l,idx)=>idx===i?{...l,description:e.target.value}:l))} />
            <select value={line.account_code} style={{ ...inp, marginBottom: 4 }}
              onChange={e => {
                const a = ACCOUNTS.find(a=>a.code===e.target.value);
                setLines(p => p.map((l,idx)=>idx===i?{...l,account_code:e.target.value,account_name:a?.name??''}:l));
              }}>
              {ACCOUNTS.map(a=><option key={a.code} value={a.code}>{a.code} – {a.name}</option>)}
            </select>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input type="number" step="0.01" placeholder="Subtotal" value={line.subtotal} style={{ ...inp, flex:1, marginBottom:0 }}
                onChange={e => setLines(p => p.map((l,idx)=>idx===i?{...l,subtotal:e.target.value}:l))} />
              <label style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, color:'var(--cream-dim)', whiteSpace:'nowrap' }}>
                <input type="checkbox" checked={line.business} onChange={e => setLines(p => p.map((l,idx)=>idx===i?{...l,business:e.target.checked}:l))} />
                Business
              </label>
            </div>
          </div>
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={{ ...modal, padding: 8 }} onClick={e => e.stopPropagation()}>
        {err
          ? <p style={{ color: 'var(--cream-dim)', padding: 16 }}>{err}</p>
          : src
            ? <img src={src} alt="Source document" style={{ width: '100%', borderRadius: 8 }} />
...
export default function LedgerPage() {
  ...
  const handleApprove = async (id: string) => {
    setApprovingId(id);
    try { await ledgerApi.approve(id); await fetchData(); }
    finally { setApprovingId(null); }
  };

  return (
    /* overflow-x:hidden prevents horizontal page blowout on mobile */
    <div className="screen" style={{ overflowX: 'hidden' }}>
      ...
      {/* Tab bar — scrolls horizontally inside itself, never overflows page */}
          ...
          {ledgerEntries.length > 0 && (
            <>
              ...
              <div style={{ fontSize:13, color:'var(--cream-dim)', marginBottom:12 }}>
                Total: <strong style={{ color:'var(--gold)' }}>${runningTotal.toFixed(2)}</strong>
              </div>

              {ledgerEntries.map(entry => (
                /* min-width:0 is essential — prevents grid children from overflowing */
                <div key={entry.id} className="ledger-row" style={{ minWidth:0 }}>
                  ...
                  {/* Col 2 — entity + actions */}
                  <div style={{ minWidth:0 }}>
                    <div className="ledger-entity">{entry.entity ?? '—'}</div>
                    <span className={`badge ${entry.status}`} style={{ marginTop:4, display:'inline-block' }}>
                    ...
                    <div style={{ display:'flex', gap:5, marginTop:6, flexWrap:'wrap' }}>
                      ...
                      <button className="btn-secondary" style={{ fontSize:10, padding:'2px 8px' }}
                        onClick={()=>setSourceEntryId(entry.id)}>📄 Doc</button>
                    </div>
                  </div>
      ...
      {!loading && view==='journal' && (
        <>
            ...
            <div key={entry.id} className="journal-card">
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                <div style={{ minWidth:0 }}>
                  ...
                  <div style={{ fontSize:16, fontWeight:700, color:'var(--cream)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {entry.entity}
                  </div>
                </div>
                <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6, flexShrink:0, marginLeft:8 }}>
                  <span className={`badge ${entry.status}`}>{entry.status==='DRAFT'?'DRAFT':entry.status}</span>
                  <button className="btn-secondary" style={{ fontSize:11 }}
                    onClick={()=>setSourceEntryId(entry.ledger_entry_id)}>📄 Source</button>
                </div>
              </div>

              {/* Journal lines table — table-layout:fixed prevents overflow */}
              <table className="journal-table">
                <thead><tr><th style={{ width:'55%' }}>ACCOUNT</th><th>DEBIT</th><th>CREDIT</th></tr></thead>
                <tbody>
                  {(entry.lines ?? []).map((line, li) => (
                    <tr key={li}>
                      <td style={{ color:'var(--cream-dim)', overflow:'hidden', textOverflow:'ellipsis' }}>
              ...
              {(entry.status === 'DRAFT' || entry.status === 'NEEDS_REVIEW') && (
                <button className="btn-approve" style={{ marginTop:10 }}
          ...
          <button className="btn-primary" style={{ marginTop:10, background:'var(--bg-card)', color:'var(--cream)' }}
